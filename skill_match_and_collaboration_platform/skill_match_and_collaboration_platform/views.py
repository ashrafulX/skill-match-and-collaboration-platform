...
def find_job(request):
    jobs = JobPost.objects.select_related('author').prefetch_related('Required_Technical_Skills')
...
def filter_jobs(request):
    ...
    if categories:
        jobs = jobs.filter(Required_Technical_Skills__id__in=categories).distinct()
    if job_types:
